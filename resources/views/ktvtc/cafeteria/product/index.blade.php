...
<script>
    ...
    async function showStockModal(productId) {
        try {
            ...
            document.getElementById('stockShopId').value = product.shop_id || '';
    ...
    async function saveProduct() {
        ...
        if (method === 'POST') {
            url = '{{ route("cafeteria.products.store") }}';
        } else {
            const productId = document.getElementById('productId').value;
            url = `/cafeteria/products/${productId}`;
        ...
        if (!document.getElementById('track_inventory').checked) {
            formData.set('track_inventory', '0');
        }
        if (!document.getElementById('is_production_item').checked) {
            formData.set('is_production_item', '0');
        }
    ...
    function previewFile(input) {
        ...
        reader.onloadend = function() {
            ...
            // Clear remove flag when new image is selected
            document.getElementById('remove_image').value = '0';
        }
