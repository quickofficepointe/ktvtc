...
<div class="grid grid-cols-1 xl:grid-cols-4 gap-6">
    ...
    <option value="{{ $category->id }}">{{ $category->category_name }}</option> {{-- Changed from name to category_name --}}
    @endforeach
</select>
    ...
    <button onclick="filterByCategory({{ $category->id }})"
        ...
        {{ $category->category_name }} {{-- Changed from name to category_name --}}
    </button>
    @endforeach
</div>
        </div>
...
<script>
    ...
    // Customer Functions
  // In your existing blade template, update the searchCustomer function:
function searchCustomer() {
    ...
    $.ajax({
        url: '{{ route("sales.search.customers") }}', // Make sure this route exists
        method: 'GET',
        data: {
            phone: phone,
...
// Also update the updateTodayStats function to use the correct route:
function updateTodayStats() {
    $.ajax({
        url: '{{ route("sales.today.stats") }}', // Make sure this route exists
        method: 'GET',
   ...
   function sendMpesaPrompt() {
    ...
    // Prepare items data for the request
    ...
    // Call M-Pesa API using KCB Buni
    $.ajax({
        url: '{{ route("sales.initiate.mpesa") }}', // Make sure this route exists
        method: 'POST',
        data: {
            amount: amount,
            phone: phone,
        ...
        success: function(response) {
            ...
            if (response.status === 'success' || response.success) {
                // Store the checkout request ID from KCB response
                ...
                if (checkoutRequestId) {
                    ...
                    startMpesaCheck(checkoutRequestId, response.data.sale_id);
                } else {
                    $('#mpesaStatusMessage').html('<span class="text-red-600">No checkout request ID received</span>');
                    $('#sendPromptBtn').prop('disabled', false);
                }
            } else {
                const errorMsg = response.message || response.error || 'Failed to send prompt';
                $('#mpesaStatusMessage').html('<span class="text-red-600">' + errorMsg + '</span>');
                $('#sendPromptBtn').prop('disabled', false);
                toastr.error('Failed to send M-Pesa prompt: ' + errorMsg);
            }
        },
        error: function(xhr, status, error) {
            console.error('M-Pesa AJAX error:', xhr.responseText);
            $('#mpesaStatusMessage').html('<span class="text-red-600">Error connecting to payment service</span>');
            $('#sendPromptBtn').prop('disabled', false);
            toastr.error('Failed to connect to payment service');
        }
    });
}

function startMpesaCheck(checkoutRequestId) {
    console.log('Starting M-Pesa check for ID:', checkoutRequestId);
    ...
    mpesaCheckInterval = setInterval(() => {
        ...
        if (attempts >= maxAttempts) {
            ...
            $('#sendPromptBtn').prop('disabled', false);
            toastr.warning('Payment timeout. Please ask customer to confirm or try again.');
            return;
        }

        console.log(`Checking M-Pesa status (attempt ${attempts}/${maxAttempts})`);

        // Check M-Pesa status with KCB Buni API
        $.ajax({
            url: '{{ route("cafeteria.sales.pos.check-mpesa-status") }}',
            method: 'POST',
            data: {
                checkout_request_id: checkoutRequestId, // This is the crucial parameter!
                _token: csrfToken
            },
            success: function(response) {
                ...
                if (response.status === 'success' || response.success) {
                    clearInterval(mpesaCheckInterval);
                    completeMpesaPayment(response.receipt_number || response.ReceiptNumber);
                } else if (response.status === 'pending') {
                    $('#mpesaStatusMessage').text(`Checking payment... (${attempts}/${maxAttempts})`);
                } else {
                    const errorMsg = response.message || response.error || 'Payment check failed';
                    $('#mpesaStatusMessage').html('<span class="text-red-600">' + errorMsg + '</span>');
                }
            },
            error: function(xhr) {
                console.error('Status check error:', xhr.responseText);
                // Don't show error to user for every failed attempt, just keep trying
            }
        });
...
function completeMpesaPayment(receiptNumber) {
    console.log('M-Pesa payment confirmed. Receipt:', receiptNumber);

    $('#mpesaStatusMessage').html('<span class="text-green-600"><i class="fas fa-check-circle mr-1"></i> Payment confirmed! Receipt: ' + receiptNumber + '</span>');

    // Disable the send button since payment is complete
    $('#sendPromptBtn').prop('disabled', true);

    setTimeout(() => {
        // Process the sale with M-Pesa payment
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.status === 'success') {
                        clearInterval(mpesaCheckInterval);
                        completeMpesaPayment(response.receipt_number);
                    } else {
                        $('#mpesaStatusMessage').text(`Checking payment... (${attempts}/${maxAttempts})`);
                    }
                }
    ...
    function processPayment(method, details) {
        ...
        const saleData = {
            ...
            mpesa_receipt: method === 'mpesa' ? details.mpesa_receipt : null,
            transaction_id: method === 'mpesa' ? details.transaction_id : null,
            _token: csrfToken
        };
    ...
    function updateTodayStats() {
        $.ajax({
            url: '{{ route("cafeteria.api.sales.today-stats") }}',
            method: 'GET',
            success: function(stats) {
                $('#todaySales').text('KES ' + (stats.total_sales || 0).toFixed(2));
                $('#todayTransactions').text(stats.transaction_count || 0);
