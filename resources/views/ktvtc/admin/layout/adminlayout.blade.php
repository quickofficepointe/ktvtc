...
<body class="bg-gray-50">
        ...
        <div class="flex items-center justify-between h-full px-4">
            <div class="flex items-center">
                ...
                <a class="flex items-center" href="{{ route('admin.dashboard') }}">
                    ...
                    <span class="font-bold text-white text-xl">KTVTC Admin <span class="text-sm bg-white bg-opacity-20 px-2 py-1 rounded ml-2">Panel</span></span>
                </a>
            </div>
    ...
    <div class="fixed top-16 left-0 bottom-0 w-64 text-white z-20 overflow-y-auto sidebar hidden md:block" id="sidebar">
        <div class="p-4">
            ...
            <ul class="space-y-2">
                <li>
                    <a href="{{ route('admin.dashboard') }}" class="flex items-center px-3 py-3 text-sm rounded-md nav-link {{ request()->routeIs('admin.dashboard') ? 'active' : '' }}">
                ...
                <li>
                    <a href="{{ route('admin.users.index') }}" class="flex items-center px-3 py-3 text-sm rounded-md nav-link {{ request()->routeIs('admin.users.*') ? 'active' : '' }}">
                        <i class="fas fa-users-cog mr-3"></i> User Management
                    </a>
                </li>

                <li class="mt-4">
                    <p class="px-3 py-2 text-xs font-semibold uppercase tracking-wider text-white text-opacity-70">APPLICATIONS</p>
                </li>
                <li>
                    <a href="{{ route('admin.applications.index') }}" class="flex items-center px-3 py-3 text-sm rounded-md nav-link {{ request()->routeIs('admin.applications.*') ? 'active' : '' }}">
                        <i class="fas fa-graduation-cap mr-3"></i> Course Applications
                    </a>
                </li>
                <li>
                    <a href="{{ route('admin.event-applications.index') }}" class="flex items-center px-3 py-3 text-sm rounded-md nav-link {{ request()->routeIs('admin.event-applications.*') ? 'active' : '' }}">
                        <i class="fas fa-calendar-check mr-3"></i> Event Applications
                    </a>
                </li>

                <li class="mt-4">
                    <p class="px-3 py-2 text-xs font-semibold uppercase tracking-wider text-white text-opacity-70">COMMUNICATIONS</p>
                </li>
                <li>
                    <a href="{{ route('admin.messages.index') }}" class="flex items-center px-3 py-3 text-sm rounded-md nav-link {{ request()->routeIs('admin.messages.*') ? 'active' : '' }}">
    <i class="fas fa-envelope mr-3"></i> Messages
    @php
        $unreadCount = \App\Models\Message::where('status', '!=', 'viewed')->count();
    @endphp
    @if($unreadCount > 0)
    ...
    @endif
                ...
                <li>
                    <a href="{{ route('admin.subscriptions.index') }}" class="flex items-center px-3 py-3 text-sm rounded-md nav-link {{ request()->routeIs('admin.subscriptions.*') ? 'active' : '' }}">
                        <i class="fas fa-newspaper mr-3"></i> Subscriptions
