    ...
    <div class="flex items-center justify-center min-h-screen px-4 py-6">
        ...
        <div class="relative bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-hidden transform transition-all">
            ...
            <div class="p-6 overflow-y-auto max-h-[calc(90vh-80px)]">
                <form action="{{ route('library.acquisition-requests.store') }}" method="POST" class="space-y-6">
                    @csrf
...
<script>
    ...
    function filterByStatus(status) {
        window.location.href = '{{ route('library.acquisition-requests.index') }}?status=' + status;
    }

    function clearFilters() {
        window.location.href = '{{ route('library.acquisition-requests.index') }}';
    }
    ...
    function approveRequest(requestId) {
        if(confirm('Approve this acquisition request?')) {
            ...
            form.method = 'POST';
            form.action = `/library/acquisition-requests/${requestId}/approve`;
    ...
    function rejectRequest(requestId) {
        if(confirm('Reject this acquisition request?')) {
            const form = document.createElement('form');
            form.method = 'POST';
            form.action = `/library/acquisition-requests/${requestId}/reject`;
    ...
    function markAsOrdered(requestId) {
        if(confirm('Mark this request as ordered?')) {
            const form = document.createElement('form');
            form.method = 'POST';
            form.action = `/library/acquisition-requests/${requestId}/mark-ordered`;
    ...
    function markAsReceived(requestId) {
        if(confirm('Mark this request as received?')) {
            const form = document.createElement('form');
            form.method = 'POST';
            form.action = `/library/acquisition-requests/${requestId}/mark-received`;
    ...
    function viewRequest(requestId) {
        window.location.href = `/library/acquisition-requests/${requestId}`;
    }
