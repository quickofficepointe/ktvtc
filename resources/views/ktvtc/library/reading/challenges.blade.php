...
<script>
    function refreshPopularities() {
        if(confirm('Refresh all popularity scores? This will recalculate based on current data.')) {
            ...
            form.method = 'POST';
            form.action = '{{ route("library.book-popularities.refresh") }}';
    ...
    function generateReport() {
        window.open('{{ route("library.book-popularities.report") }}', '_blank');
    }
    ...
    function updatePopularity(popularityId) {
        if(confirm('Update this book\'s popularity score?')) {
            const form = document.createElement('form');
            form.method = 'POST';
            form.action = `/library/book-popularities/${popularityId}/update`;
