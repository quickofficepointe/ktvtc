    ...
    <div class="flex items-center justify-center min-h-screen px-4 py-6">
        ...
        <div class="relative bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-hidden transform transition-all">
            ...
            <div class="p-6 overflow-y-auto max-h-[calc(90vh-80px)]">
                <form action="{{ route('library.notifications.store') }}" method="POST" class="space-y-6">
                    @csrf
...
<script>
    ...
    function markAllAsRead() {
        if(confirm('Mark all notifications as read?')) {
            const form = document.createElement('form');
            form.method = 'POST';
            form.action = '{{ route("library.notifications.mark-all-read") }}';
    ...
    function filterByStatus(status) {
        window.location.href = '{{ route('library.notifications.index') }}?status=' + status;
    }

    function filterByType(type) {
        window.location.href = '{{ route('library.notifications.index') }}?type=' + type;
    }

    function clearFilters() {
        window.location.href = '{{ route('library.notifications.index') }}';
    }

    function viewNotification(notificationId) {
        window.location.href = `/library/notifications/${notificationId}`;
    }
