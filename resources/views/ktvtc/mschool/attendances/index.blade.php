                            ...
                            <td class="px-6 py-4 align-top">
                                <div class="flex flex-col gap-2">
                                    <div class="flex gap-2">
                                        ...
                                        <button onclick="confirmDelete('{{ route('mschool.attendances.destroy', $attendance->attendance_id) }}')" class="px-3 py-1 text-xs rounded bg-red-50 text-red-600 hover:bg-red-100">Delete</button>
                                    </div>
                                    <div class="flex gap-2">
                                        <a href="{{ route('mschool.attendances.records', $attendance->attendance_id) }}"
                                           class="px-3 py-1 text-xs rounded bg-green-50 text-green-600 hover:bg-green-100 text-center">
                                            Records
                                        </a>
                                        @if($attendance->is_locked)
                                            <form action="{{ route('mschool.attendances.unlock', $attendance->attendance_id) }}" method="POST" class="inline">
                                                @csrf
                                        ...
                                        @else
                                            <form action="{{ route('mschool.attendances.lock', $attendance->attendance_id) }}" method="POST" class="inline">
                                                @csrf
                                    ...
                                    @if($attendance->recording_method === 'qr_code')
                                        <form action="{{ route('mschool.attendances.generate-qrcode', $attendance->attendance_id) }}" method="POST" class="inline">
                                            @csrf
                                    ...
                                    @endif
                                    <form action="{{ route('mschool.attendances.update-statistics', $attendance->attendance_id) }}" method="POST" class="inline">
                                        @csrf
    ...
    <div class="flex items-center justify-center min-h-screen px-4 py-6">
        ...
        <div class="relative bg-white rounded-2xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-hidden transform transition-all">
            ...
            <div class="p-6 overflow-y-auto max-h-[calc(90vh-80px)]">
                <form id="createForm" action="{{ route('mschool.attendances.store') }}" method="POST">
                    @csrf
