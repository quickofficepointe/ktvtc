...
<script>
    function openGalleryModal(galleryId) {
        ...
        document.getElementById('modalGalleryTitle').textContent = 'Loading...';
        document.getElementById('modalGalleryCount').textContent = '';
        document.getElementById('modalGalleryContent').innerHTML = `
            <div class="flex justify-center py-12">
                <div class="loading-spinner"></div>
                <p class="ml-3 text-gray-600">Loading gallery...</p>
            </div>
        `;
        ...
        // Fetch gallery data from the new API endpoint
        fetch(`/api/galleries/${galleryId}`)
            ...
            .then(gallery => {
                ...
                document.getElementById('modalGalleryCount').textContent = `${gallery.image_count} photos`;

                // Build gallery content
                let content = `
                    <div class="space-y-6">
                        ${gallery.description ? `
                        <div class="bg-gray-50 rounded-lg p-4 border-l-4 border-[#B91C1C]">
                    ...
                    content += `
                        <div class="group relative bg-white rounded-lg overflow-hidden shadow-md hover:shadow-xl transition-all duration-300 transform hover:scale-105">
                            <img src="/storage/${image.image_path}"
                                 ...
                                 onclick="openImageLightbox('/storage/${image.image_path}', ${index}, ${gallery.images.length})">
            ...
            .catch(error => {
                ...
                document.getElementById('modalGalleryContent').innerHTML = `
                    <div class="text-center py-12">
                        ...
                        <h3 class="text-lg font-semibold text-gray-800 mb-2">Error Loading Gallery</h3>
                        <p class="text-gray-600">Unable to load gallery. Please try again.</p>
                    </div>
                `;
                document.getElementById('modalGalleryTitle').textContent = 'Error';
                document.getElementById('modalGalleryCount').textContent = '';
            });
    }
    ...
    function openImageLightbox(imagePath, currentIndex, totalImages) {
        // You can implement a lightbox here, for now open in new tab
