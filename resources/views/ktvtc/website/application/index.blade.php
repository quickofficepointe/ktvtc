...
<div>
            ...
            @if($pastIntakes->count() > 0)
                <optgroup label="Past Intakes (Manual Applications)">
                    @foreach($pastIntakes as $intake)
                        <option value="{{ $intake->month }} {{ $intake->year }}"
                                class="text-gray-500">
                            {{ $intake->month }} {{ $intake->year }} (Past Intake)
                        </option>
                    @endforeach
                </optgroup>
            @endif
    ...
    @if(isset($pastIntakes) && $pastIntakes->count() > 0)
        <p class="text-sm text-amber-600 mt-2 flex items-start">
            ...
            <span>Past intakes are available for manual/backdated applications. Select a past intake if you're applying for a previous period.</span>
        </p>
    @endif
</div>
    ...
    setTimeout(() => {
        try {
            ...
            if (selectedCampusOption) {
                const campusHeading = selectedCampusOption.querySelector('h4');
                campusName = campusHeading ? campusHeading.textContent : '-';
            }
            ...
            const idType = idTypeSelect ? idTypeSelect.options[idTypeSelect.selectedIndex]?.text || '-' : '-';
            ...
            const educationLevel = educationLevelSelect ? educationLevelSelect.options[educationLevelSelect.selectedIndex]?.text || '-' : '-';
            ...
            const applicationTypeSelect = document.querySelector('select[name="application_type"]');
            const applicationType = applicationTypeSelect ? applicationTypeSelect.options[applicationTypeSelect.selectedIndex]?.text || '-' : '-';
            ...
            const intakePeriodSelect = document.querySelector('select[name="intake_period"]');
            const intakePeriod = intakePeriodSelect ? intakePeriodSelect.options[intakePeriodSelect.selectedIndex]?.text || '-' : '-';

            const studyModeSelect = document.querySelector('select[name="study_mode"]');
            const studyMode = studyModeSelect ? studyModeSelect.options[studyModeSelect.selectedIndex]?.text || '-' : '-';
            ...
            const idDocumentName = idDocument && idDocument.files.length > 0 ? idDocument.files[0].name : 'Not uploaded';

            const educationCertificates = document.getElementById('educationCertificates');
            const educationCertificatesName = educationCertificates && educationCertificates.files.length > 0 ? educationCertificates.files[0].name : 'Not uploaded';

            const passportPhoto = document.getElementById('passportPhoto');
            const passportPhotoName = passportPhoto && passportPhoto.files.length > 0 ? passportPhoto.files[0].name : 'Not uploaded';
            ...
            const reviewHTML = `
                ...
                <div class="bg-gray-50 rounded-lg p-6 border border-gray-200">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                        ...
                        <div>
                            <span class="text-gray-600">Study Mode:</span>
                            <p class="font-semibold text-gray-800">${studyMode}</p>
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <p class="font-semibold text-gray-800">${applicationType}</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-gray-50 rounded-lg p-6 border border-gray-200">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                        ...
                        <div>
                            ...
                            <p class="font-semibold text-gray-800">${idType}</p>
                        </div>
                        <div>
                ...
                <div class="bg-gray-50 rounded-lg p-6 border border-gray-200">
                    ...
                    <ul class="space-y-2 text-sm">
                        <li class="flex items-center">
                            ...
                            <span>${idType} - ${idDocumentName}</span>
                        </li>
                        <li class="flex items-center">
                            <svg class="w-4 h-4 text-green-600 mr-2" fill="currentColor" viewBox="0 0 20 20">
                                <path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd"/>
                            </svg>
                            <span>Education Certificates - ${educationCertificatesName}</span>
                        </li>
                        <li class="flex items-center">
                            <svg class="w-4 h-4 text-green-600 mr-2" fill="currentColor" viewBox="0 0 20 20">
                                <path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd"/>
                            </svg>
                            <span>Passport Photo - ${passportPhotoName}</span>
                        </li>
                    </ul>
                </div>
            `;
            ...
            reviewContent.innerHTML = `
                <div class="bg-red-50 border border-red-200 rounded-lg p-6 text-center">
                    ...
                    <h4 class="text-lg font-semibold text-red-800 mb-2">Error Loading Review</h4>
                    <p class="text-red-700">Please go back and check all steps are complete.</p>
                </div>
            `;
        }
    }, 500);
