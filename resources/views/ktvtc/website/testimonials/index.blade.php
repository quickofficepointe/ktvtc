...
<div class="container mx-auto px-4 py-8">
    ...
    <div class="flex justify-between items-center mb-8">
        <div>
            ...
            <p class="text-gray-600 mt-2">Manage student and visitor testimonials</p>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    ...
                    <p class="text-2xl font-bold text-gray-900">{{ $testimonials->count() }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    ...
                    <p class="text-2xl font-bold text-gray-900">{{ $testimonials->where('is_approved', true)->count() }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    ...
                    <p class="text-2xl font-bold text-gray-900">{{ $testimonials->where('is_approved', false)->count() }}</p>
                </div>
            </div>
        </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-200">
            <h2 class="text-lg font-semibold text-gray-900">All Testimonials</h2>
        </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full">
                <thead class="bg-gray-50">
                    <tr>
                        <th class="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Testimonial Details</th>
                        ...
                        <tr class="hover:bg-gray-50 transition-colors duration-150">
                            <td class="px-6 py-4">
                                <div class="flex items-start space-x-4">
                                    ...
                                    @else
                                        <div class="w-12 h-12 rounded-full bg-gradient-to-br from-primary to-red-400 flex items-center justify-center shadow-sm">
                                    ...
                                    <div class="flex-1 min-w-0">
                                        ...
                                        <p class="text-sm text-gray-600 mt-1 line-clamp-2">{{ $testimonial->review }}</p>
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                            ...
                            <td class="px-6 py-4">
                                <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium {{ $testimonial->is_approved ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800' }}">
                                    <span class="w-2 h-2 rounded-full mr-2 {{ $testimonial->is_approved ? 'bg-green-400' : 'bg-yellow-400' }}"></span>
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center space-x-2">
                                    @if(!$testimonial->is_approved)
                                        <form action="{{ route('website.testimonials.approve', $testimonial->id) }}" method="POST" class="inline">
                                            @csrf
                                            <button type="submit"
                                                    class="inline-flex items-center px-3 py-2 border border-transparent shadow-sm text-sm leading-4 font-medium rounded-md text-white bg-green-600 hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 transition-colors">
                                    ...
                                    @endif
                                    <button onclick="confirmDelete('{{ route('website.testimonials.destroy', $testimonial->id) }}')"
    ...
    <div class="flex items-center justify-center min-h-screen px-4 py-6">
        ...
        <div class="relative bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-hidden transform transition-all">
            ...
            <div class="bg-gradient-to-r from-primary to-red-600 px-6 py-4">
                <div class="flex items-center justify-between">
                    <div class="flex items-center space-x-3">
                        ...
                        <div>
                            <h3 class="text-xl font-bold text-white">Add New Testimonial</h3>
                            <p class="text-red-100 text-sm">Add a student or visitor testimonial</p>
                        </div>
                    </div>
            ...
            <div class="p-6 overflow-y-auto max-h-[calc(90vh-80px)]">
                <form action="{{ route('website.testimonials.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                    @csrf
                    ...
                    <div>
                        ...
                        <textarea name="review" rows="4" required
                                  class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:ring-2 focus:ring-primary focus:border-transparent transition-colors"
                                  placeholder="Enter the testimonial content">{{ old('review') }}</textarea>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="flex items-center space-x-1" id="rating-stars">
                            @for($i = 1; $i <= 5; $i++)
                                <button type="button"
                                        ...
                                        data-rating="{{ $i }}"
                                        onclick="setRating({{ $i }})">
                                    <svg class="w-8 h-8 text-gray-300 rating-icon"
                                         data-rating="{{ $i }}"
                                         fill="currentColor"
                                         viewBox="0 0 20 20">
                                        <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/>
                        ...
                        <input type="hidden" name="rating" id="selected-rating" value="{{ old('rating', 5) }}" required>
                        <p class="text-sm text-gray-500 mt-2" id="rating-text">Selected: 5 stars - Excellent</p>
                    </div>
                        ...
                        <div class="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-lg">
                            <div class="space-y-1 text-center">
                                ...
                                <div class="flex text-sm text-gray-600">
                                    <label for="image_path" class="relative cursor-pointer bg-white rounded-md font-medium text-primary hover:text-red-500 focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-primary">
...
<script>
    ...
    function openCreateModal() {
        ...
        resetRating(5); // Default to 5 stars
    }
    ...
    function setRating(rating) {
        ...
        ratingText.textContent = `Selected: ${ratingTexts[rating]}`;
    }

    function resetRating(rating = 5) {
        setRating(rating);
    }
