                        ...
                        <tr class="hover:bg-gray-50 transition-colors duration-150">
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center space-x-2">
                                    ...
                                    <button onclick="confirmDelete('{{ route('events.destroy', $event->id) }}')"
    ...
    function openEditModal(eventId) {
        fetch(`/events/${eventId}`)
            ...
            .then(data => {
                ...
                document.getElementById('editShortDescription').value = data.short_description || '';
                document.getElementById('editDescription').value = data.description || '';
                document.getElementById('editEventType').value = data.event_type || '';
                document.getElementById('editDepartment').value = data.department || '';
                document.getElementById('editEventStartDate').value = data.event_start_date ? new Date(data.event_start_date).toISOString().slice(0, 16) : '';
                document.getElementById('editEventEndDate').value = data.event_end_date ? new Date(data.event_end_date).toISOString().slice(0, 16) : '';
                document.getElementById('editRegistrationStartDate').value = data.registration_start_date ? new Date(data.registration_start_date).toISOString().slice(0, 16) : '';
                document.getElementById('editRegistrationEndDate').value = data.registration_end_date ? new Date(data.registration_end_date).toISOString().slice(0, 16) : '';
                document.getElementById('editLocation').value = data.location || '';
                document.getElementById('editTargetAudience').value = data.target_audience || 'all_students';
                document.getElementById('editIsPaid').checked = data.is_paid;
                document.getElementById('editPrice').value = data.price || '';
                document.getElementById('editEarlyBirdPrice').value = data.early_bird_price || '';
                document.getElementById('editEarlyBirdEndDate').value = data.early_bird_end_date ? new Date(data.early_bird_end_date).toISOString().slice(0, 16) : '';
                document.getElementById('editMaxAttendees').value = data.max_attendees || '';
                document.getElementById('editRegisteredAttendees').value = data.registered_attendees || 0;
                document.getElementById('editOrganizerName').value = data.organizer_name || '';
                document.getElementById('editOrganizerEmail').value = data.organizer_email || '';
                document.getElementById('editOrganizerPhone').value = data.organizer_phone || '';
                document.getElementById('editOrganizerWebsite').value = data.organizer_website || '';
                ...
                document.getElementById('editPriceFields').classList.toggle('hidden', !data.is_paid);
            })
            .catch(error => {
