...
<script>
    ...
    function openEditModal(id, name, description, isActive) {
        ...
        // Set form action
        document.getElementById('editForm').action = `/blogcategories/${id}`;
...
// Update openEditModal function to show current image
function openEditModal(id, name, description, isActive, coverImage = null) {
    const modal = document.getElementById('editModal');
    modal.classList.remove('hidden');
    document.body.style.overflow = 'hidden';

    // Set form action
    document.getElementById('editForm').action = `/blog-categories/${id}`;
    ...
    if (coverImage) {
        currentImage.src = `/storage/${coverImage}`;
