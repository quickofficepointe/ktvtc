...
<head>
    ...
    <title>@yield('title', 'Super Admin Dashboard') - KTVTC</title>
        ...
        tailwind.config = {
            theme: {
                extend: {
                    colors: {
                        'kenswed-red': '#BF1F30',
                        'kenswed-red-light': '#D34554',
                        'kenswed-red-dark': '#9A1826',
                        'kenswed-black': '#1A1A1A',
                        'kenswed-gray': '#4A4A4A',
                        'danger': '#BF1F30',
                        'warning': '#F4A261',
                        'success': '#2A9D8F',
    ...
    <style>
        ...
        /* Sidebar */
        .kenswed-sidebar {
            background: linear-gradient(180deg, #BF1F30 0%, #9A1826 100%);
            box-shadow: 4px 0 15px rgba(191, 31, 48, 0.2);
        }

        .kenswed-sidebar.collapsed {
            width: 70px;
        }

        .kenswed-sidebar.collapsed .nav-text,
        .kenswed-sidebar.collapsed .group-title {
            display: none;
        }
        ...
        .kenswed-nav-link:hover {
            background: rgba(255, 255, 255, 0.15);
            transform: translateX(5px);
        }

        .kenswed-nav-link.active {
            background: linear-gradient(90deg, rgba(255, 255, 255, 0.2) 0%, transparent 100%);
            color: white !important;
            border-left: 4px solid white;
        }

        .kenswed-nav-link.active::after {
            content: '';
            position: absolute;
            right: 0;
            top: 50%;
        ...
        .badge-super {
            background: linear-gradient(135deg, #BF1F30 0%, #D34554 100%);
            color: white;
        }
        ...
        .kenswed-card {
            background: white;
            border-radius: 12px;
            box-shadow: 0 4px 20px rgba(191, 31, 48, 0.08);
            transition: all 0.3s ease;
            border: 1px solid rgba(191, 31, 48, 0.1);
        }

        .kenswed-card:hover {
            transform: translateY(-5px);
            box-shadow: 0 8px 30px rgba(191, 31, 48, 0.15);
        }
        ...
        .kenswed-table tbody tr {
            transition: all 0.2s ease;
        }

        .kenswed-table tbody tr:hover {
            background: linear-gradient(90deg, rgba(191, 31, 48, 0.05) 0%, transparent 100%);
            transform: scale(1.005);
        }

        /* Custom Scrollbar */
        .kenswed-scrollbar::-webkit-scrollbar {
            width: 6px;
            height: 6px;
        }

        .kenswed-scrollbar::-webkit-scrollbar-track {
            background: #F1F1F1;
            border-radius: 10px;
        }

        .kenswed-scrollbar::-webkit-scrollbar-thumb {
            background: linear-gradient(135deg, #BF1F30 0%, #9A1826 100%);
            border-radius: 10px;
        }

        .kenswed-scrollbar::-webkit-scrollbar-thumb:hover {
            background: linear-gradient(135deg, #9A1826 0%, #BF1F30 100%);
        }
        ...
        /* Progress Bar */
        .progress-bar {
            background: linear-gradient(90deg, #BF1F30 0%, #D34554 100%);
        }
        ...
        input:checked + .kenswed-slider {
            background: linear-gradient(135deg, #BF1F30 0%, #D34554 100%);
        }

        input:checked + .kenswed-slider:before {
...
<body class="min-h-screen bg-gray-50">
    ...
    <nav class="fixed top-0 left-0 right-0 h-16 bg-white shadow-lg z-40 border-b border-gray-200">
        <div class="flex items-center justify-between h-full px-4 lg:px-6">
            ...
            <div class="flex items-center space-x-4">
                ...
                <div class="flex items-center">
                    <img src="{{ asset('Assets/images/Kenswed_logo.png') }}"
                         alt="Kenswed College Logo"
                         class="w-12 h-12 object-contain mr-3"
                         onerror="this.onerror=null; this.src='https://placehold.co/48x48/BF1F30/ffffff?text=K';">
                    <div>
                        <h1 class="font-bold text-gray-800 text-lg">KTVTC <span class="text-[#BF1F30]">Super Admin</span></h1>
            ...
            <div class="flex items-center space-x-4">
                <!-- System Status -->
                <div class="hidden md:flex items-center space-x-3 px-4 py-2 bg-[#BF1F30]/5 rounded-lg">
                    <div class="relative">
                ...
                <div class="relative">
                    <button id="notificationBtn" class="relative p-2 rounded-lg text-gray-600 hover:bg-gray-100">
                        ...
                        <span class="absolute -top-1 -right-1 w-5 h-5 bg-[#BF1F30] text-white text-xs rounded-full flex items-center justify-center font-bold">
                            3
                        </span>
                    </button>
                    ...
                    <div id="notificationDropdown" class="hidden absolute right-0 mt-2 w-96 bg-white rounded-xl shadow-2xl border border-gray-200 z-50">
                        <div class="p-4 border-b border-gray-200">
                            <div class="flex items-center justify-between">
                                ...
                                <button class="text-xs text-[#BF1F30] hover:text-[#9A1826] font-medium">
                                    <i class="fas fa-trash-alt mr-1"></i> Clear All
                                </button>
                            </div>
                        </div>
                        <div class="max-h-96 overflow-y-auto kenswed-scrollbar">
                        ...
                        <div class="p-4 border-t border-gray-200 bg-gray-50">
                            <a href="{{ route('super-admin.system.logs') }}" class="block text-center text-[#BF1F30] hover:text-[#9A1826] font-medium">
                ...
                <!-- Quick Actions -->
                <div class="relative">
                    <button id="quickActionsBtn" class="px-4 py-2 bg-gradient-to-r from-[#BF1F30] to-[#9A1826] text-white rounded-lg font-semibold hover:shadow-lg transition-shadow flex items-center space-x-2">
                    ...
                    <div id="quickActionsDropdown" class="hidden absolute right-0 mt-2 w-72 bg-white rounded-xl shadow-2xl border border-gray-200 z-50">
                        ...
                        <div class="p-2">
                            <a href="{{ route('super-admin.users.create') }}" class="flex items-center p-3 rounded-lg hover:bg-gray-50 mb-2">
                                <div class="w-10 h-10 rounded-lg bg-[#BF1F30]/10 flex items-center justify-center mr-3">
                                    <i class="fas fa-user-shield text-[#BF1F30]"></i>
                                </div>
                                <div>
                ...
                <div class="relative">
                    <button id="userDropdownBtn" class="flex items-center space-x-3 p-2 rounded-lg hover:bg-gray-100">
                        <div class="w-10 h-10 rounded-full bg-gradient-to-br from-[#BF1F30] to-[#9A1826] flex items-center justify-center shadow-lg">
                            <i class="fas fa-user-shield text-white"></i>
                        </div>
                        <div class="text-left hidden lg:block">
                            <p class="text-sm font-bold text-gray-800">{{ Auth::user()->name ?? 'Super Admin' }}</p>
                            <div class="flex items-center">
                                <span class="kenswed-badge badge-super">SUPER ADMIN</span>
                    ...
                    <div id="userDropdown" class="hidden absolute right-0 mt-2 w-64 bg-white rounded-xl shadow-2xl border border-gray-200 z-50">
                        <div class="p-4 border-b border-gray-200">
                            <div class="flex items-center">
                                <div class="w-12 h-12 rounded-full bg-gradient-to-br from-[#BF1F30] to-[#9A1826] flex items-center justify-center mr-3">
                        ...
                        <div class="p-2">
                            <a href="{{ route('super-admin.system.settings') }}" class="flex items-center p-3 rounded-lg hover:bg-gray-50 mb-1">
                                <i class="fas fa-sliders-h text-[#BF1F30] mr-3"></i>
                            ...
                            <a href="{{ route('profile.edit') }}" class="flex items-center p-3 rounded-lg hover:bg-gray-50 mb-1">
                                <i class="fas fa-user-edit text-[#BF1F30] mr-3"></i>
                            ...
                            <a href="{{ route('super-admin.system.logs') }}" class="flex items-center p-3 rounded-lg hover:bg-gray-50 mb-1">
                                <i class="fas fa-history text-[#BF1F30] mr-3"></i>
                                <span class="font-medium text-gray-700">Activity Logs</span>
                            </a>
                            <div class="border-t border-gray-200 mt-2 pt-2">
                                <form method="POST" action="{{ route('logout') }}" class="w-full">
                                    @csrf
                                    <button type="submit" class="flex items-center justify-center w-full p-3 rounded-lg hover:bg-red-50 text-[#BF1F30] font-semibold group">
    ...
    <!-- Sidebar -->
    <aside id="sidebar" class="kenswed-sidebar fixed top-16 left-0 bottom-0 w-64 text-white pt-6 z-30 overflow-y-auto kenswed-scrollbar">
        <!-- Search -->
        <div class="px-4 mb-6">
            <div class="relative">
                <input type="text"
                       placeholder="Search commands..."
                       class="w-full pl-10 pr-4 py-2.5 bg-white/10 border border-white/20 rounded-xl text-white placeholder-white/50 focus:outline-none focus:ring-2 focus:ring-white/50 focus:border-transparent">
        ...
        <nav class="px-4 space-y-1">
            ...
            <a href="{{ route('super-admin.dashboard') }}"
               class="kenswed-nav-link flex items-center px-4 py-3 rounded-xl mb-1 {{ request()->routeIs('super-admin.dashboard') ? 'active' : '' }}">
            ...
            <!-- USER MANAGEMENT -->
            <div class="mt-8 mb-3">
                <p class="px-4 py-2 text-xs font-bold text-white/70 uppercase tracking-widest group-title">
                    <i class="fas fa-users mr-2"></i>User Management
                </p>
            </div>

            <a href="{{ route('super-admin.users.index') }}"
               class="kenswed-nav-link flex items-center px-4 py-3 rounded-xl {{ request()->routeIs('super-admin.users.*') && !request()->routeIs('super-admin.users.create') ? 'active' : '' }}">
            ...
            <a href="{{ route('super-admin.users.create') }}"
               class="kenswed-nav-link flex items-center px-4 py-3 rounded-xl {{ request()->routeIs('super-admin.users.create') ? 'active' : '' }}">
            ...
            <a href="{{ route('super-admin.roles.index') }}"
               class="kenswed-nav-link flex items-center px-4 py-3 rounded-xl {{ request()->routeIs('super-admin.roles.*') ? 'active' : '' }}">
            ...
            <a href="{{ route('super-admin.system.settings') }}"
               class="kenswed-nav-link flex items-center px-4 py-3 rounded-xl {{ request()->routeIs('super-admin.system.settings') ? 'active' : '' }}">
                <i class="fas fa-cogs mr-3 text-lg"></i>
                <span class="nav-text">System Settings</span>
            </a>

            <a href="{{ route('super-admin.system.database') }}"
               class="kenswed-nav-link flex items-center px-4 py-3 rounded-xl {{ request()->routeIs('super-admin.system.database') ? 'active' : '' }}">
                <i class="fas fa-database mr-3 text-lg"></i>
                <span class="nav-text">Database</span>
            </a>

            <a href="{{ route('super-admin.system.logs') }}"
               class="kenswed-nav-link flex items-center px-4 py-3 rounded-xl {{ request()->routeIs('super-admin.system.logs') ? 'active' : '' }}">
                <i class="fas fa-history mr-3 text-lg"></i>
                <span class="nav-text">System Logs</span>
                <span class="ml-auto bg-white/30 text-white text-xs px-2 py-1 rounded-lg animate-pulse">
            ...
            <div class="space-y-1">
                ...
                @foreach($modules as $module)
                    <a href="{{ route($module['route']) }}"
                       class="kenswed-nav-link flex items-center px-4 py-3 rounded-xl hover:bg-white/15">
                        ...
                        <span class="ml-auto text-white/70 text-xs">
            ...
            <a href="{{ route('super-admin.reports.audit') }}"
               class="kenswed-nav-link flex items-center px-4 py-3 rounded-xl {{ request()->routeIs('super-admin.reports.*') ? 'active' : '' }}">
                <i class="fas fa-clipboard-list mr-3 text-lg"></i>
                <span class="nav-text">Audit Trail</span>
            </a>

            <a href="{{ route('super-admin.reports.activity') }}"
               class="kenswed-nav-link flex items-center px-4 py-3 rounded-xl">
            ...
            <a href="#" class="kenswed-nav-link flex items-center px-4 py-3 rounded-xl">
        ...
        <!-- Sidebar Footer -->
        <div class="absolute bottom-0 left-0 right-0 p-4 border-t border-white/20 bg-[#9A1826]/50">
            <div class="text-center">
                <div class="text-xs text-white/70 mb-2">Super Admin Session</div>
                <div class="flex items-center justify-center space-x-2">
                    <div class="w-2 h-2 rounded-full bg-success animate-pulse"></div>
                    <div class="text-xs text-white/90">Active • {{ now()->format('H:i') }}</div>
                </div>
            </div>
        </div>
    </aside>
    ...
    <main id="mainContent" class="ml-0 lg:ml-64 pt-16 min-h-screen bg-gray-50">
        <!-- Breadcrumb -->
        <div class="bg-gradient-to-r from-[#BF1F30]/5 to-[#9A1826]/5 border-b border-gray-200 px-6 py-4">
            <div class="flex flex-col md:flex-row md:items-center md:justify-between">
                <div>
                    <nav class="flex" aria-label="Breadcrumb">
                        <ol class="inline-flex items-center space-x-1 md:space-x-3">
                            <li class="inline-flex items-center">
                                <a href="{{ route('super-admin.dashboard') }}" class="inline-flex items-center text-sm font-medium text-[#BF1F30] hover:text-[#9A1826]">
        ...
        <div class="px-6 pt-6">
            ...
            @if(session('error'))
                <div class="mb-6 p-4 bg-gradient-to-r from-[#BF1F30]/10 to-[#BF1F30]/5 border-l-4 border-[#BF1F30] rounded-r-lg animate-fade-in">
                    <div class="flex items-center">
                        <div class="w-10 h-10 rounded-full bg-[#BF1F30]/20 flex items-center justify-center mr-3">
                            <i class="fas fa-exclamation-circle text-[#BF1F30]"></i>
                        </div>
                        <div>
        ...
        <footer class="border-t border-gray-200 bg-white px-6 py-4 mt-8">
            <div class="flex flex-col md:flex-row md:items-center md:justify-between">
                <div class="text-gray-600 text-sm">
                    <p class="font-medium">
                        <i class="fas fa-shield-alt text-[#BF1F30] mr-1"></i>
    ...
    <script>
        ...
        window.showToast = function(message, type = 'success', duration = 5000) {
            ...
            const colors = {
                ...
                error: { bg: 'bg-[#BF1F30]/10', border: 'border-[#BF1F30]', icon: 'fa-exclamation-circle', text: 'text-[#BF1F30]' },
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.querySelectorAll('.kenswed-nav-link').forEach(link => {
