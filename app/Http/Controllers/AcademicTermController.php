...
class AcademicTermController extends Controller
{
    ...
    public function adminindex(Request $request)
    {
        ...
        // Get existing academic years from the database for the filter dropdown
        $existingAcademicYears = AcademicTerm::select('academic_year')
            ->distinct()
            ->orderBy('academic_year', 'desc')
            ->pluck('academic_year');

        // Generate a range of years for the create form (current year -2 to +2)
        $currentYear = date('Y');
        $academicYearsForForm = range($currentYear - 2, $currentYear + 2);

        // For the filter dropdown, use existing years if available, otherwise use the generated range
        $academicYears = $existingAcademicYears->isNotEmpty()
            ? $existingAcademicYears
            : collect($academicYearsForForm);

        // Add termNumbers for the create modal
        $termNumbers = [1, 2, 3, 4];
    ...
    public function store(Request $request)
    {
        $validator = Validator::make($request->all(), [
            ...
            'is_fee_generation_locked' => 'boolean', // ADDED THIS LINE
        ...
        try {
            ...
            // If it's an AJAX request, return JSON response
            if ($request->wantsJson() || $request->ajax()) {
                return response()->json(['success' => true, 'message' => 'Academic term created successfully.']);
            }
            ...
            // If it's an AJAX request, return JSON response
            if ($request->wantsJson() || $request->ajax()) {
                return response()->json(['error' => 'Failed to create academic term: ' . $e->getMessage()], 500);
            }
