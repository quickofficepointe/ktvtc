...
class StudentController extends Controller
{
    public function dashboard()
    {
        ...
        return view('ktvtc.students.dashboard', compact('isApproved'));
    }
    ...
    public function update(Request $request, Student $student)
    {
        ...
        \Log::info('Updating student', [
            ...
            'current_id_number' => $student->id_number,
            'new_id_number' => $request->id_number
        ]);

        // Validation rules WITHOUT any unique constraints
        $rules = [
            // Personal Information
            ...
            // Email - NO UNIQUE VALIDATION for updates
            'email' => 'nullable|email|max:150',

            // Phone
            'phone' => 'nullable|string|max:20',

            // ID Number - NO UNIQUE VALIDATION for updates
            'id_number' => 'nullable|string|max:30',

            // Student Number - NO UNIQUE VALIDATION for updates
        ...
        try {
            ...
            // Handle application_id specially - if empty, set to null
            if (empty($data['application_id'])) {
            ...
            if ($request->hasFile('other_documents')) {
                ...
                if ($student->other_documents) {
                    $existingDocs = is_string($student->other_documents)
                        ? json_decode($student->other_documents, true)
                        : (is_array($student->other_documents) ? $student->other_documents : []);
                }
            ...
            // If application_id changed, handle the relationship
            if ($student->wasChanged('application_id')) {
                if ($student->getOriginal('application_id')) {
                    Application::where('id', $student->getOriginal('application_id'))
                        ->update(['student_id' => null]);
                }

                if ($student->application_id) {
                    Application::where('id', $student->application_id)
    ...
    public function editDetails(Student $student)
    {
        ...
        return view('ktvtc.admin.students.edit', compact('student'));
    }
    ...
    public function updateDetails(Request $request, Student $student)
    {
        ...
        return redirect()->route('students.details', $student)
    ...
    public function downloadTemplate()
    {
        ...
        return Excel::download(
            new class($headings, $sampleData) implements
                ...
                \Maatwebsite\Excel\Concerns\WithStyles
        {
            ...
            public function __construct($headings, $sampleData)
            {
            ...
            public function styles(\PhpOffice\PhpSpreadsheet\Worksheet\Worksheet $sheet)
            {
                ...
                $sheet->getDelegate()->getParent()->createSheet();
                $sheet->getDelegate()->getParent()->setActiveSheetIndex(1);
                $instructionSheet = $sheet->getDelegate()->getParent()->getActiveSheet();
                ...
                $instructions = [
                    ...
                    [],
                    ['The import system recognizes multiple header variations. Here are examples:'],
                    [],
                ];
                ...
                $mappingExamples = [
                    ...
                    ['phone', 'phone, Phone, Telephone, Mobile, Contact'],
                    ['id_number', 'id_number, ID Number, National ID, ID No'],
                    ['gender', 'gender, Gender, Sex (M/F/Male/Female)'],
                    ['date_of_birth', 'date_of_birth, DOB, Birth Date, Birthday'],
                    ['student_category', 'student_category, Category, Student Type'],
                    ['status', 'status, Status, Student Status'],
                    ['student_number', 'student_number, Student No, Student ID, Admission No'],
                ];

                $row = 1;
                foreach ($instructions as $instructionRow) {
                    $instructionSheet->setCellValue('A' . $row, $instructionRow[0] ?? '');
                    $row++;
                }

                foreach ($mappingExamples as $example) {
                    $instructionSheet->setCellValue('A' . $row, $example[0]);
                    $instructionSheet->setCellValue('B' . $row, $example[1]);
                    $row++;
                }

                $row += 2;
                $instructionSheet->setCellValue('A' . $row, 'VALIDATION RULES:');
                $row++;
                $instructionSheet->setCellValue('A' . $row, '- Gender: male, female, other, M, F');
                $row++;
                $instructionSheet->setCellValue('A' . $row, '- ID Type: id, birth_certificate, passport');
                $row++;
                $instructionSheet->setCellValue('A' . $row, '- Status: active, inactive, graduated, dropped, suspended, alumnus, prospective, historical');
                $row++;
                $instructionSheet->setCellValue('A' . $row, '- Student Category: regular, alumnus, staff_child, sponsored, scholarship');
                $row++;
                $instructionSheet->setCellValue('A' . $row, '- Dates: YYYY-MM-DD format (e.g., 2024-01-15)');
                ...
                $sheet->getDelegate()->getParent()->setActiveSheetIndex(0);
            }
        },
    ...
    public function enrollmentReport(Request $request)
    {
        ...
        // FIXED: Use raw expressions in groupBy and orderBy instead of the alias
        $query = Student::query()
            ->select(DB::raw('DATE(registration_date) as date'), DB::raw('COUNT(*) as count'))
            ->groupBy(DB::raw('DATE(registration_date)'))
            ->orderBy(DB::raw('DATE(registration_date)'), 'asc');
        ...
        // For the count queries, we need a fresh query without the groupBy
        $baseQuery = Student::query()
            ->when($user->role != 2, function ($q) use ($user) {
                return $q->where('campus_id', $user->campus_id);
            })
            ->when($request->filled('date_from'), function ($q) use ($request) {
                return $q->whereDate('registration_date', '>=', $request->date_from);
            })
            ->when($request->filled('date_to'), function ($q) use ($request) {
                return $q->whereDate('registration_date', '<=', $request->date_to);
            })
            ->when($request->filled('campus_id') && $user->role == 2, function ($q) use ($request) {
                return $q->where('campus_id', $request->campus_id);
            });

        // These counts should be on the students themselves, not on the grouped data
        $totalEnrolled = (clone $baseQuery)->count();

        $enrolledThisMonth = (clone $baseQuery)
            ->whereMonth('registration_date', now()->month)
            ->whereYear('registration_date', now()->year)
            ->count();

        $enrolledThisYear = (clone $baseQuery)
            ->whereYear('registration_date', now()->year)
            ->count();
