...
class StockService
{
     ...
     */
    public function updateStock($productId, $shopId, $quantity, $type, $reason = null, $notes = null)
    {
        DB::beginTransaction();
        ...
        try {
            ...
            $movement = $this->createMovement($product, $shopId, $type, $quantity, $oldStock, $newStock, $reason, $notes);
    ...
    public function reserveStock($productId, $shopId, $quantity, $referenceId, $referenceType = 'sale')
    {
        ...
        $stock = InventoryStocks::where('product_id', $productId)
     ...
     */
    private function createMovement($product, $shopId, $type, $quantity, $oldStock, $newStock, $reason, $notes)
    {
    ...
    private function syncInventoryStock($product, $shopId, $currentStock)
    {
        try {
            ...
            $inventoryStock = InventoryStocks::where('product_id', $product->id)
                ->where('shop_id', $shopId)
                ->first();
            ...
            return InventoryStocks::where('product_id', $product->id)
                ->where('shop_id', $shopId)
                ->first();
        }
    }
