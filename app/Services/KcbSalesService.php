...
class KcbSalesService extends KcbService
{
        ...
        try {
            if ($resultCode == 0) {
                ...
                if ($sale) {
                    ...
                    $this->sendPaymentSmsNotifications($sale);
                ...
                if ($sale) {
                    ...
                    // 🔔 OPTIONAL: Send failure SMS to customer
                    // $this->sendPaymentFailedSms($sale);
                }
    ...
    /**
     * Send SMS notifications after successful payment
     * This method handles different payment types (cafeteria, fees, events, etc.)
     */
    private function sendPaymentSmsNotifications($sale)
    {
        try {
            $smsService = app(SmsService::class);

            // Determine payment type based on sale details
            $paymentType = $this->determinePaymentType($sale);

            switch ($paymentType) {
                case 'cafeteria_order':
                    // Send customer order confirmation
                    $customerSms = $this->generateCafeteriaCustomerSms($sale);
                    $smsService->sendSingleSms($sale->customer_phone, $customerSms);

                    // Send admin notification
                    $adminSms = $this->generateCafeteriaAdminSms($sale);
                    foreach ($smsService->getAdminPhones() as $adminPhone) {
                        $smsService->sendSingleSms($adminPhone, $adminSms);
                    }
                    break;

                case 'fee_payment':
                    // Send student fee payment confirmation
                    $studentSms = $this->generateFeePaymentSms($sale);
                    $smsService->sendSingleSms($sale->customer_phone, $studentSms);

                    // Send accounts/admin notification
                    $accountsSms = $this->generateFeeAdminSms($sale);
                    foreach ($smsService->getAdminPhones() as $adminPhone) {
                        $smsService->sendSingleSms($adminPhone, $accountsSms);
                    }
                    break;

                case 'event_payment':
                    // Send event registration confirmation
                    $attendeeSms = $this->generateEventPaymentSms($sale);
                    $smsService->sendSingleSms($sale->customer_phone, $attendeeSms);
                    break;

                default:
                    // Generic payment confirmation
                    $genericSms = "Kenswed: Payment of KES " . number_format($sale->total_amount, 0) .
                                  " for {$sale->invoice_number} confirmed. Receipt: {$sale->mpesa_receipt}. Thank you! www.ktvtc.ac.ke";
                    $smsService->sendSingleSms($sale->customer_phone, $genericSms);
            }

            Log::info('Payment SMS notifications sent', [
                'sale_id' => $sale->id,
                'type' => $paymentType,
            ...
            Log::error('Failed to send payment SMS notifications: ' . $e->getMessage(), [
                'sale_id' => $sale->id
            ]);
        }
    }
    ...
    /**
     * Determine payment type from sale
     */
    private function determinePaymentType($sale)
    {
        // Check if it's a cafeteria order (has shop_id and items)
        if ($sale->shop_id && $sale->items()->count() > 0) {
            return 'cafeteria_order';
        }

        // Check if it's a fee payment (has enrollment_id or student_id)
        if ($sale->enrollment_id || $sale->student_id) {
            return 'fee_payment';
        }

        // Check if it's an event payment
        if ($sale->event_id || $sale->event_application_id) {
    ...
    /**
     * Generate cafeteria customer SMS (short format with all items)
     */
    private function generateCafeteriaCustomerSms($sale)
    {
        // Build items list
        $itemsList = [];
        ...
        // Estimated ready time (25 minutes from now)
        $readyTime = now()->addMinutes(25)->format('g:i A');

        return "Kenswed: Order {$sale->invoice_number} confirmed! {$itemsText}. Total KES " .
               number_format($sale->total_amount, 0) . ". M-Pesa Receipt: {$sale->mpesa_receipt}. " .
               "Ready by {$readyTime}. Thank you! www.ktvtc.ac.ke";
    }

    /**
     * Generate cafeteria admin SMS (detailed with all info)
     */
    private function generateCafeteriaAdminSms($sale)
    {
        $shop = $sale->shop;
        $shopName = $shop ? $shop->shop_name : 'Cafeteria';

        // Build items with prices
        $itemsWithPrices = [];
        foreach ($sale->items as $item) {
            $itemsWithPrices[] = $item->product_name . ' x' . $item->quantity .
                                ' (KES ' . number_format($item->final_price, 0) . ')';
        }
        $itemsText = implode(', ', $itemsWithPrices);

        $customerInfo = $sale->customer_name . ' (' . $sale->customer_phone;
        if ($sale->customer_email) {
            $customerInfo .= ', ' . $sale->customer_email;
        }
        $customerInfo .= ')';

        return "🔔 NEW ORDER - {$shopName} • Invoice: {$sale->invoice_number} • " .
               "Customer: {$customerInfo} • " .
               "Items: {$itemsText} • Total: KES " . number_format($sale->total_amount, 0) . " • " .
               "Payment: M-PESA {$sale->mpesa_receipt} (PAID) • Ready by " . now()->addMinutes(25)->format('g:i A');
    }

    /**
     * Generate fee payment SMS for student
     */
    private function generateFeePaymentSms($sale)
    {
        return "Kenswed: Fee payment of KES " . number_format($sale->total_amount, 0) .
               " for {$sale->invoice_number} received. Receipt: {$sale->mpesa_receipt}. " .
               "Thank you! www.ktvtc.ac.ke";
    }

    /**
     * Generate fee payment admin SMS for accounts
     */
    private function generateFeeAdminSms($sale)
    {
        return "💰 FEE PAYMENT • Invoice: {$sale->invoice_number} • " .
               "Student: {$sale->customer_name} ({$sale->customer_phone}) • " .
               "Amount: KES " . number_format($sale->total_amount, 0) . " • " .
               "M-PESA: {$sale->mpesa_receipt} • Time: " . now()->format('d/m/Y H:i');
    }

    /**
     * Generate event payment SMS
     */
    private function generateEventPaymentSms($sale)
    {
        return "Kenswed: Event registration payment of KES " . number_format($sale->total_amount, 0) .
               " confirmed. Receipt: {$sale->mpesa_receipt}. Thank you! www.ktvtc.ac.ke";
    }
