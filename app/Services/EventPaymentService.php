...
class EventPaymentService
{
    ...
    public function processEventApplicationAndPayment($data, $event)
    {
        try {
            ...
            // 🔔 Send confirmation SMS for free event
            $this->sendFreeEventConfirmationSms($application);
    ...
    public function handleEventPaymentCallback($callbackData)
    {
        ...
        if ($resultCode == 0) {
            ...
            if ($application) {
                ...
                $this->sendEventPaymentSuccessSms($application);
            }
            ...
            if ($application) {
                ...
                // 🔔 OPTIONAL: Send payment failure SMS to client
                $this->sendEventPaymentFailedSms($application);
            }
    ...
    /**
     * Send SMS notifications after successful event payment
     */
    private function sendEventPaymentSuccessSms($application)
    {
        try {
            $event = $application->event;

            // 1. Send confirmation SMS to client
            $clientMessage = "Kenswed: Event registration confirmed for {$event->title}! ";
            $clientMessage .= "Invoice: EVT-{$application->id}, Total KES " . number_format($application->total_amount, 0) . ". ";
            $clientMessage .= "M-Pesa Receipt: {$application->mpesa_reference_number}. ";
            $clientMessage .= "Number of attendees: {$application->number_of_people}. ";
            $clientMessage .= "Thank you! www.ktvtc.ac.ke";

            $clientResult = $this->smsService->sendSingleSms($application->parent_contact, $clientMessage);

            Log::info('Event client SMS sent', [
                'application_id' => $application->id,
                'success' => $clientResult['success'] ?? false
            ]);

            // 2. Send notification SMS to admin (254790148509)
            $adminMessage = "🎟️ NEW EVENT REGISTRATION - {$event->title}\n";
            $adminMessage .= "Client: {$application->parent_name} ({$application->parent_contact})\n";
            $adminMessage .= "Attendees: {$application->number_of_people}\n";
            $adminMessage .= "Amount: KES " . number_format($application->total_amount, 0) . "\n";
            $adminMessage .= "Receipt: {$application->mpesa_reference_number}\n";
            $adminMessage .= "Time: " . now()->format('d/m/Y H:i');

            // Send to the admin number provided
            $this->smsService->sendSingleSms('254790148509', $adminMessage);

            // Also send to any other admin phones from SmsService
            foreach ($this->smsService->getAdminPhones() as $adminPhone) {
                if ($adminPhone !== '254790148509') { // Avoid duplicate
                    $this->smsService->sendSingleSms($adminPhone, $adminMessage);
                }
            }

        } catch (\Exception $e) {
            Log::error('Failed to send event payment SMS: ' . $e->getMessage(), [
                'application_id' => $application->id
            ]);
        }
    }
    ...
    /**
     * Send free event confirmation SMS
     */
    private function sendFreeEventConfirmationSms($application)
    {
        try {
            $event = $application->event;

            // Send confirmation to client
            $clientMessage = "Kenswed: Free event registration confirmed for {$event->title}! ";
            $clientMessage .= "Attendees: {$application->number_of_people}. ";
            $clientMessage .= "Thank you! www.ktvtc.ac.ke";

            $this->smsService->sendSingleSms($application->parent_contact, $clientMessage);

            // Send admin notification
            $adminMessage = "🎟️ FREE EVENT REGISTRATION - {$event->title}\n";
            $adminMessage .= "Client: {$application->parent_name} ({$application->parent_contact})\n";
            $adminMessage .= "Attendees: {$application->number_of_people}\n";
            $adminMessage .= "Time: " . now()->format('d/m/Y H:i');
            ...
            Log::error('Failed to send free event SMS: ' . $e->getMessage());
        }
    }
     ...
     */
    private function sendEventPaymentFailedSms($application)
    {
        try {
            $event = $application->event;

            $clientMessage = "Kenswed: Payment for {$event->title} was unsuccessful. ";
            $clientMessage .= "Please try again or contact support. ";
            $clientMessage .= "www.ktvtc.ac.ke";
    ...
    public function cancelApplication($applicationId)
    {
        try {
            ...
            try {
                $event = $application->event;
                $cancelMessage = "Kenswed: Your registration for {$event->title} has been cancelled. ";
                $cancelMessage .= "If this was a mistake, please register again. www.ktvtc.ac.ke";
                $this->smsService->sendSingleSms($application->parent_contact, $cancelMessage);
            } catch (\Exception $e) {
    ...
    public function verifyAndUpdatePayment($applicationId, $mpesaReferenceNumber)
    {
        try {
            ...
            // Send confirmation SMS for manual verification
            $this->sendEventPaymentSuccessSms($application);
